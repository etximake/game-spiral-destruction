...
## 🔑 Key Facts (Config-Driven Architecture)
- All mode parameters are in `Modes/SpiralDestruction/config.json` (inside the mode folder)
- `Core/Base/ModeConfigLoader.gd` parses and validates JSON configs
- `GameManager.MODE_CONFIG_REGISTRY` maps mode_id → config path
- `SimulationBase.set_config(config)` passes config to modes
- Subsystems get config via their own `set_config()` methods
- Current test radii: [45, 38, 25, 17, 12], speeds: [320, 390, 470, 560, 680]
